...
scoreboard.renderer = {
    ...
    init: function () {
        ...
        var commanderKeys = scoreboard
            .datastore
            .getCommanderKeys();
        for (i = 0; i < commanderKeys.length; i++) {
    ...
    redraw: function () {
        ...
        var playerKeys = scoreboard
            .datastore
            .getPlayerKeys();
        var commanderKeys = scoreboard
            .datastore
            .getCommanderKeys();
        var showPoison = scoreboard
            .datastore
            .isActive('Poison');
        var showCommanderDamage = scoreboard
            .datastore
            .isActive('CommanderDamage');
        var showCommanderImages = scoreboard
            .datastore
            .isActive('CommanderImages');
        ...
        // setup headers. uesd json properties: name (string), iconClass (string),
        // buttons (bool), clickFunction, commanderName (string)
        var headers = [
            {
                name: 'Player',
        ...
        if (showCommanderDamage) {
            for (i = 0; i < commanderKeys.length; i++) {
                headers.push({
                    name: scoreboard
                        .datastore
                        .getCommanderName(commanderKeys[i]),
        ...
        for (i = 0; i < headers.length; i++) {
            ...
            if (typeof(headers[i].commanderName) != 'undefined') {
                ...
                if (showCommanderImages) {
                    ...
                    $('#mainContainer table thead tr th:last div.commanderImage').attr('id', 'image_' + headers[i].commanderName);

                    scoreboard
                        .cardinfo
                        .getCardImage(headers[i].name, this._getImageCallback(headers[i].commanderName));
                }
            }
        ...
        for (i = 0; i < playerKeys.length; i++) {
            ...
            if (showCommanderDamage) {
                for (j = 0; j < commanderKeys.length; j++) {
                    columns.push({
                        name: 'commanderDamage_' + playerKeys[i] + '_' + commanderKeys[j],
                        value: scoreboard
                            .datastore
                            .getCommanderDamage(playerKeys[i], commanderKeys[j]),
    ...
    _getImageCallback: function (commanderName) {
        return function (cardInfo) {
            var gradient = 'linear-gradient(rgba(255, 255, 255, .0), rgba(255, 255, 255, .05), rgba(255, 255' +
                    ', 255, .8), rgba(255, 255, 255, 1.2))';
    ...
    _getRemoveCommanderFunction: function (commanderKey) {
        var commanderName = scoreboard
            .datastore
            .getCommanderName(commanderKey);
        var message = 'Are you sure you want to remove ' + commanderName + ' and the damage it dealt?';
        return function () {
            if (confirm(message)) {
                scoreboard
                    .functions
                    .removeCommander(commanderKey);
            }
        };
    },
    ...
    _getPromptFunction: function (type, playerName, commanderName) {
        ...
        if (type == 'commanderDamage') {
            result = function (amount) {
                scoreboard
                    .functions
                    .setCommanderDamage(playerName, commanderName, amount);
            };
        }
        return (function () {
            scoreboard
                .renderer
                .promptForAmount(result);
        });
    },
    ...
    _getClickFunction: function (type, playerName, commanderName) {
        ...
        if (type == 'damage') {
            result = function (amount) {
                scoreboard
                    .functions
                    .incrementDamage(playerName, amount);
            };
        }
        if (type == 'poison') {
            result = function (amount) {
                scoreboard
                    .functions
                    .incrementPoison(playerName, amount);
            };
        }
        if (type == 'commanderDamage') {
            result = function (amount) {
                scoreboard
                    .functions
                    .incrementCommanderDamage(playerName, commanderName, amount);
            };
        }
        return (result);
    },
    ...
    _getProgressWidth: function (current, min, max) {
        ...
        if (current >= max) 
            ...
            width = Math.floor(100 * current / max);
        return (width);
    },
    ...
    redrawCommanderDamage: function (playerKey, commanderKey) {
        var current = scoreboard
            .datastore
            .getCommanderDamage(playerKey, commanderKey);
        ...
        $('#progressBar_commanderDamage_' + playerKey + '_' + commanderKey)
            .removeClass()
            .addClass('progress-bar')
            .addClass(className);
    },
    ...
    promptForAmount: function (callback) {
        bootbox
            .prompt('Set to which value?', function (result) {
    ...
    getCommanderFormData: function () {
        ...
        return ({commanderName: commanderName, commanderInfect: commanderInfect});
    },
